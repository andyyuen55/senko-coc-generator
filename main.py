...
# ✨ 新增：讓 Word 自動尋找所有上傳的注意事項圖片並換行貼上
doc.add_paragraph('{% for img in NOTES_IMAGES %}{{ img }}\n{% endfor %}')
...
p = doc.add_paragraph()
run = p.add_run('Label Format:')
run.bold = True

doc.add_paragraph('{{ LABEL_FORMATS }}')
doc.add_paragraph('標籤圖示說明 (Label Reference)：')
# ✨ 新增：讓 Word 自動尋找所有上傳的標籤圖片並換行貼上
doc.add_paragraph('{% for img in LABEL_IMAGES %}{{ img }}\n{% endfor %}')
